...
- **Config graceful degradation**: most `config.Load()` errors are swallowed at startup, and most `config.LoadFrom()` failures during reconciliation are logged as warnings — reconcile works without a project config file. Invalid `exec` post-sync hooks are the narrow fail-closed exception: startup or a GitOps reload aborts before deployment. The reconciler re-reads `bosun.yaml` from the repo after each git pull to pick up hook changes
