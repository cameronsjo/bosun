...
- `site/` — raw Astro static site deployed to <https://cameronsjo.github.io/bosun> by `.github/workflows/pages.yml` (build-only on PRs; build+deploy on main push). **Rollback is `git revert` on `main`**, not a `workflow_dispatch` from an older ref: the `github-pages` environment's deployment branch policy is main-only, so a dispatch from any other ref builds and is then rejected at the deploy job. That branch policy is a load-bearing security control — widening it would let any account with repo write access publish arbitrary branch content to the public site. Deploy contract is asserted by `internal/workflowcontract/pages_test.go` (`make test-workflows`), but the branch policy lives in repo settings and no test can pin it. `site/scripts/sync-docs.mjs` copies an explicit allowlist (six core docs + `docs/adr/*`) into the content collection at build time — extend the allowlist there, never with a directory glob.
- Diagrams are dual-layer: `docs/diagrams/*.mmd` + README ASCII (`make diagrams`) are the bot/terminal layer; `docs/diagrams/*.{html,svg}` are the committed editorial layer, redrawn via the diagram-design skill under the `bosun` profile (`docs/styles/diagram-profile.md` canonical, `.diagram-design` marker). `make diagrams-check` (CI lint job) enforces two things: each export embeds its source `.mmd` sha256 and fails when a `.mmd` changes without a re-export, and the exported SVG carries no active content (script, event handlers, `foreignObject`, non-fragment `href`, `javascript:` URLs) — the site inlines these SVGs via `set:html`, so the hash alone cannot catch a hand-edited export. After changing a `.mmd`, re-redraw and re-run `node scripts/diagrams/export-svg.mjs`.
...
| `BOSUN_HOOK_SETTLE_DELAY` | daemon, reconcile | Global pause before post-sync hooks run. When neither file nor environment configures a value and the effective local deploy path is exactly `/mnt/user` or beneath it, bosun applies a 2-second fallback. Explicit file or environment values always win, including `0s`; an unconfigured non-`/mnt/user` path retains zero delay |
...
| `BOSUN_HEALTH_GATE_TIMEOUT` | daemon, reconcile | Health gate polling timeout (default: `60s`; `0` disables) |
| `BOSUN_HEALTH_GATE_SCOPE` | daemon, reconcile | What the post-compose-up health gate polls and rolls back on: `critical` (default — only `critical_containers` members), `declared` (all declared services, with the #392 pre-existing-casualty exemption), or `off` (no gate). Unknown values are ignored with a warning. Overrides config file `health_gate_scope` |
...
| `BOSUN_SKIP_DEPLOY_INVARIANT` | daemon, reconcile | Bypass the post-deploy created/written path existence, type, fresh-mtime, and no-file-write content checks (default: `false`). Set to `true` for diagnostic deploys where silent-sync failures are acceptable. Logged at `Warn` with `override=true` when enabled. |
...
- **Async trigger reconciles are daemon-owned** — socket, TCP, API, webhook, manual, and drift self-heal handlers register through `startReconcileGoroutine`, inherit daemon shutdown cancellation without inheriting request cancellation, and join `d.wg`. Tests that use these handlers must wait with `waitForDaemonTasks` before temp-dir cleanup; do not restore flag polling or detached goroutines
- **Unix socket mode is applied before publication** — `SocketConfig.SocketMode` and `daemon.Config.SocketMode` default to `0660`. On Unix, `listenUnixSocket` binds under a private staging directory, chmods there, then atomically publishes into place; do not replace this with process-global `umask` or final-path `Listen` then `Chmod`. Cleanup is inode-checked and stale symlinks/non-sockets are rejected
...
- **Config graceful degradation**: most `config.Load()` errors are swallowed at startup, and most `config.LoadFrom()` failures during reconciliation are logged as warnings — reconcile works without a project config file. Invalid `exec` post-sync hooks are the narrow fail-closed exception: startup or a GitOps reload aborts before deployment. The reconciler re-reads `bosun.yaml` from the repo after each git pull to pick up hook changes. A valid empty file is an authoritative snapshot; a missing/malformed file retains current hook state
- **Hook reload presence**: after a successful snapshot, absent/empty root `post_sync_hooks` clears file hooks, absent `hook_settle_delay` retains its effective value, and explicit `0s` clears it. Target omission inherits root, explicit `[]` clears, and removed descriptors fall back to root until the required topology restart
- **Env var precedence**: `BOSUN_POST_SYNC_HOOKS` (JSON) completely *replaces* hooks from `bosun.yaml`, it does not merge. Target hooks supplied by `BOSUN_TARGETS` are environment-owned too
...
- **Hook paths are staging-relative**: Content-hash sync (`CopyDirIfChanged`) returns paths relative to each deploy target dir and `deployLocal` prefixes them with `t.RelPath`. Hook globs use staging-relative paths (e.g., `appdata/authelia/**`); git-diff fallback strips the configured infra prefix into that same namespace before matching
