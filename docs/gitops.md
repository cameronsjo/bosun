...
The breaker acts only inside a resolved Docker Compose project: a single target's `project_name`, and otherwise the root-level `project_name` in `bosun.yaml` (the directory-name fallback does not count, because it names no deployed project). With no scope, the breaker stops nothing — an unscoped stop would reach every restart-looping container on the Docker host, including containers Bosun never deployed. That disabled state is announced at daemon startup, again on every drift cycle, and by `bosun doctor`. The periodic drift comparison itself keeps its own, host-wide scope and is unaffected.
...
| `BOSUN_RESTART_BREAKER` | No | `true` | Enable restart-loop protection (requires a resolved compose project scope) |
