...
- **2026-09-08 — Task 3b's transport spike is closed as not-feasible, and the transfer bound leaves this change.** The plan's preferred design ("a custom `transport.Transport` registered for `ssh` that dials the TCP connection itself") cannot be built: `transport.Transport.NewUploadPackSession` returns an `UploadPackSession`, and go-git v5.19.2's only implementation of that session layer is `common.NewClient` in `plumbing/transport/**internal**/common`, which bosun cannot import. The exported `ssh.NewClient(config)` is the `overrideConfig` field-blanking path the plan already declined. Bosun can bound the **TCP dial** and nothing after it. The plan's own fallback applies — ship 3a, state the residual — with one correction: the residual is *dial bounded, handshake and transfer not*, because `ssh.NewClientConn` honors no deadline either. Filed as [#655](https://github.com/cameronsjo/bosun/issues/655) with the four options ranked. This is the spec gate doing its job: the boundary surfaced before an implementer spent a day on it.
