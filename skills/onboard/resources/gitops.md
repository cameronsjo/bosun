...
```text
 ...
 8. Deploy files (local copy or tar-over-SSH). Remote deploys use a
    ...
    live target. The remote host needs POSIX `sh` plus `tar`, `find`, `wc`,
    `tr`, `readlink`, and `sha256sum`.
...
Requires SSH key authentication and POSIX `sh` plus `tar`, `find`, `wc`, `tr`,
`readlink`, and `sha256sum` on the remote host. Test connectivity first:
`ssh user@host exit`.
