...
verification uses a process-owned mode-0700 `${TMPDIR:-/tmp}/bosun-deploy-*`
workspace containing the archive and a full extracted round trip, so plan for
temporary free space of roughly twice the source-tree size in addition to the
existing staging tree. Normal success and error paths remove the workspace;
after an unclean Bosun or host crash, operators may remove stale
`bosun-deploy-*` workspaces owned by the Bosun service account once no reconcile
process is running.
