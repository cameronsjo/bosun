...
By default reads from the daemon's cached drift state. With one configured named target, drift automatically uses that target's daemon-written state file (for example, `deploy-state-nas.json`). An explicit `--state-file` always reads exactly that path. Use `--live` for a fresh Docker check.
...
Missing or otherwise unknown deployment state exits nonzero. JSON output reports `"status": "unknown"` with an `"error"` field so scripts cannot interpret unknown state as clean.
