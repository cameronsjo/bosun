...
┌──────────────────┐     ┌─────────────────┐     ┌──────────────┐
│                  │     │                 │     │              │
│   Drift Verify   │     │  Alert + Retry  │     │ Alert + Stop │
│                  │     │                 │     │              │
└─────────┬────────┘     └─────────────────┘     └──────────────┘
          │                                                      
          │                                                      
...
A cross-process `flock` (`LOCK_EX`, non-blocking) guards the reconcile so a second
process aborts immediately. Within the daemon, single-flight reconciliation with
dirty-flag coalescing means a trigger arriving mid-run sets a pending flag and
returns `202`; the loop then performs exactly one follow-up run, so N concurrent
triggers collapse to a single extra reconcile.
...
┌───────────────────────────┐     ┌─────────────────────────────┐
│                           │     │                             │
│  set pending, return 202  │     │ acquire flock, non-blocking │
│                           │     │                             │
└───────────────────────────┘     └──────────────┬──────────────┘
                                                 │               
                                                 │               
