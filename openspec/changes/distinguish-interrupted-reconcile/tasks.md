...
- [x] 1.1 Add an optional, backward-compatible `last_attempt_outcome` state
  object for the canonical `interrupted` outcome, commit, and timestamp.
- [x] 1.2 Add state round-trip, omitted-field, legacy-file, and subsequent
  terminal-outcome clearing tests.

## 2. Classify and Account for Cancellation

- [x] 2.1 Add a pure classifier that requires both a cancelled caller context
  and a returned error wrapping `context.Canceled`; keep deadline and ambiguous
  race cases on the failure path.
- [x] 2.2 Snapshot and restore the pre-attempt commit, count, and alert-throttle
  budget for classified interruptions without clearing an existing
  `needs_redeploy` marker.
- [x] 2.3 Apply interruption finalization once at the run boundary across sync,
...
- [x] 2.4 Cover same-commit prior failures, first run of a new commit, no-effect
...
- [x] 2.5 Stop multi-target iteration whenever the shared cycle context reports
...
- [x] 3.1 Send exactly one existing-format deploy-failure alert with a canonical
  interrupted reason for each classified interruption when failure alerts are
  enabled.
- [x] 3.2 Make the run-boundary finalizer the exclusive owner of interruption
...
- [x] 3.3 Preserve context values while detaching caller cancellation and
  enforcing the 30-second maximum delivery budget.
- [x] 3.4 Verify interruption delivery bypasses attempt throttling, does not
  mutate `last_alerted_attempt`, and leaves persisted outcome intact when a
  provider fails.
- [x] 3.5 Add deterministic stage-table and health-gate tests proving propagated
...
- [x] 4.1 Update `docs/alerting.md` and
  `skills/onboard/resources/gitops.md` with interruption, retry, breaker, and
  bounded alert-delivery behavior.
- [x] 4.2 Run focused reconcile and daemon lifecycle tests, full tests, race
