...
- [ ] 1.3 Install one per-target lifecycle finalizer when render preparation begins
  so every later error and panic hardens-or-deletes staging; re-raise panics after
  recording the security outcome
- [ ] 1.4 Keep the clear-before-render replacement point so each effective target
...
- [ ] 2.1 Create the effective staging root with mode `0700` before decrypted output
...
- [ ] 2.2 Add a confined, no-follow staging verifier/hardener that requires the root
...
- [ ] 2.3 On preparation, verification, or hardening failure, delete only the
  ...
  material
- [ ] 2.4 Apply the same harden-or-delete fallback when normal cleanup fails after
  successful verification

## 3. Target isolation and repeated runs

- [ ] 3.1 Canonicalize all effective staging paths and reject the entire target set
...
- [ ] 3.2 Exercise valid implicit-default, named-derived, and explicit staging paths
  so cleanup and retention affect only the current target's effective `StagingDir`
- [ ] 3.3 Before the first post-upgrade reconciliation and idempotently before later
...
- [ ] 3.4 Verify a later render replaces prior evidence without accumulating
...
- [ ] 4.2 Cover successful rollback, failed/partial rollback, post-deploy verification
  ...
  failure
- [ ] 4.3 Verify the staging root stays `0700` from creation through deploy
...
- [ ] 4.5 Verify structured logs distinguish retained, discarded, replaced, and
  cleanup-fallback outcomes with target identity and path, but contain no rendered
  secret content or symlink target
- [ ] 4.6 Cover successful verification followed by cleanup failure: hardening
...
- [ ] 4.7 Preseed secured evidence, inject sync, config-reload, SOPS-decryption, and
  deploy-mode-resolution failures, and prove content and modes of that slot and all
  sibling slots remain unchanged
- [ ] 4.8 Run focused repeated and race tests, relevant full package tests, `go vet
