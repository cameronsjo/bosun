...
- [x] 1.1 Add a single Git authentication resolver that validates
...
- [x] 1.2 Apply the resolved Basic authentication to both `GitOps.Clone` and
  `GitOps.Pull` without transmitting it over plaintext HTTP or SSH.
- [x] 1.3 Enforce an authenticated redirect policy: allow only same-origin HTTPS
  hops and reject downgrade or cross-origin redirects without forwarding the
  Authorization header.
- [x] 1.4 Sanitize repository URLs and transport errors before every reconcile
  log/trace, validation diagnostic, returned error, daemon health/status field,
  and `/config` response.
- [x] 1.5 Validate the shared contract in standalone `bosun reconcile`, daemon
  `ValidateConfig` before servers/loops start, and every `bosun validate` path.
- [x] 1.6 Keep credentials environment-only with no YAML fields, legacy aliases,
...
- [x] 2.1 Add in-process HTTPS Git server tests (using a test-only trusted
  certificate/client) proving authenticated clone and authenticated fetch send
  the expected Basic credentials.
- [x] 2.2 Test anonymous public HTTPS remains unchanged and authentication
  rejection returns an actionable error.
- [x] 2.3 Test missing username/token pairs, credentials on non-HTTPS URLs, and
...
- [x] 2.4 Test authenticated HTTPS redirects: same-origin HTTPS is allowed,
  while downgrade and cross-origin destinations receive no Authorization header
  and synchronization fails safely.
- [x] 2.5 Test standalone reconcile, daemon startup/reconcile, and all
  ...
  precedence.
- [x] 2.6 Inject recognizable username/token/userinfo values plus their escaped
...
- [x] 2.7 Test credentials are absent from config/state serialization and project
  config reload cannot define or rotate them; document/test restart-based
  rotation semantics.
- [x] 2.8 Run focused tests repeatedly and under the race detector.

## 3. Documentation

- [x] 3.1 Document `BOSUN_GIT_USERNAME` and `BOSUN_GIT_TOKEN`, the required pair,
...
- [x] 3.2 Update `skills/onboard/resources/configuration.md` and
  `skills/onboard/resources/gitops.md` with the same contract.
- [x] 3.3 Update generated/setup deployment surfaces (`internal/cmd/init.go`,
...
- [x] 4.1 Run `go test -p=1 ./... -count=1`.
- [x] 4.2 Run relevant race tests, `go vet ./...`, `go build ./...`, and
  `golangci-lint run --timeout=5m --new-from-rev=origin/main ./...`.
- [x] 4.3 Run `openspec validate add-https-git-auth --strict` and full strict
  OpenSpec validation.
