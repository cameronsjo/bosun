...
**Unregistered target policy:** `UnmarshalYAML` accepts any top-level key with a map value into `Targets` — it does not validate against the registry at parse time. This is intentional: provisions/templates are reusable across contexts, and a target that's unregistered in one context may be registered in another. Validation happens at output time: `WriteOutputs` and `showDiff` skip unregistered targets with a warning logged via zerolog listing each skipped target name; `RenderToYAML` includes them for diagnostic visibility. Merging (`mergeProvision`, `mergeOutput`) operates on all targets regardless of registry — this preserves data through the pipeline so output-stage decisions can be made with full context.
