...
- [ ] 1.5 Keep the existing decompressed-size bound, corrupt-archive handling,
...
- [ ] 2.1 Migrate `RollbackFromBackupSet` in
  `internal/reconcile/rollback.go` from `extractBackupArchive` to
  `safeExtractBackup(<backupPath>/configs.tar.gz)`.
- [ ] 2.2 Add a caller-level valid-archive test proving the managed tree is
  restored and compose is invoked only after extraction succeeds.
- [ ] 2.3 Add caller-level traversal, absolute/escaping-symlink, and
...
- [ ] 2.4 Add caller-level extraction-context deadline/cancellation and
...
- [ ] 2.5 Preserve the rollback-not-attempted outward contract while returning an
...
- [ ] 3.1 Migrate `ComposeUpIsolated` in `internal/reconcile/compose.go` from
...
- [ ] 3.2 Add a caller-level valid-archive test proving a failed compose file is
...
- [ ] 3.3 Add caller-level traversal, absolute/escaping-symlink, and
  escaping-hardlink cases proving no backup-based compose command or orphan-pass
  use occurs when extraction fails.
- [ ] 3.4 Add caller-level extraction-context deadline/cancellation and
  partial-extraction cases using the independently bounded, background-derived
  ...
  invocation.
- [ ] 3.5 Preserve the original per-file compose failure in the result and
...
- [ ] 4.1 Migrate direct `extractBackupArchive` tests in
  `internal/reconcile/backup_test.go` to shared-extractor or caller-level coverage.
- [ ] 4.2 Search production and test code for `extractBackupArchive`; remove the
...
- [ ] 5.1 Update `skills/onboard/resources/gitops.md` to document uniform local
...
- [ ] 5.2 Update `docs/gitops.md` and `docs/error-handling.md` with the operator
  behavior for rejected or cancelled rollback archives without claiming #450's
  deferred subtree policy.
- [ ] 5.3 Update `docs/security.md` so rollback archive traversal and link-target
...
- [ ] 5.4 Treat tasks 5.1-5.3 as implementation-PR merge gates; do not update
  those consumer docs in this spec-only PR to advertise unshipped behavior.

## 6. Verification and delivery

- [ ] 6.1 Run focused rollback/extractor tests plus `make test`, `make build`, and
  repository lint/format/freshness checks.
- [ ] 6.2 Run literal Cadence polish on the implementation diff, fix every
