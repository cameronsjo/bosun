...
// running post-sync hooks. Explicit file/environment values always win,
// including zero. Only an unconfigured default zero receives the safe FUSE
// fallback for an Unraid /mnt/user target.
func resolveHookSettleDelay(configured ConfigField[time.Duration], deployPath string) time.Duration {
	if configured.Value > 0 || configured.Source != SourceDefault {
