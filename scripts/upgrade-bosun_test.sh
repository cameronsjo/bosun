...
for role in incumbent candidate; do
  ...
  for required in 'BOSUN_INFRA_DIR: "unraid"' 'DRY_RUN: "true"' 'appdata-empty:/mnt/appdata:ro' \
      'age-key.txt:/config/age-key.txt:ro' 'cap_drop: [ALL]' 'no-new-privileges:true' 'network_mode: bridge' \
      '- /run/bosun'; do
...
if [[ -n "$REAL_DOCKER" ]] && "$REAL_DOCKER" compose version >/dev/null 2>&1; then
  ...
  [[ "$(jq -r '.services.bosun.tmpfs | join(",")' <<<"$merged")" == /run/bosun ]] ||
    fail "shadow does not tmpfs the lock dir, so root cannot take the reconcile lock"
